...
from .schemas import Config, ConfigFile, DEFAULT_CONFIG_PROFILES
...
def _load_dotenv_files():
    ...
    for parent in [current_dir] + list(current_dir.parents):
        for env_file in reversed(env_files):  # Load in reverse order so higher priority files override
    ...
    if isinstance(config_dict, dict):
        ...
        def replace_env_var(match):
            ...
            if env_value is None:
                raise MissingEnvironmentVariableError(f"Environment variable '{var_name}' not found")
            return env_value
...
def _resolve_profile(config_file: ConfigFile, profile_override: Optional[str] = None) -> Tuple[Config, str]:
    """
