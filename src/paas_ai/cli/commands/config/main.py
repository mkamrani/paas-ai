    ...
    try:
        ...
        if profile:
            ...
            try:
                ...
                logger.error("No configuration file found. Use 'paas-ai config init' to create one.")
                return
...
@click.option("--profile", type=click.Choice(['local', 'default', 'production']), 
              default='local',
...
def list_profiles():
    ...
    try:
        ...
        builtin_profiles = {k: v for k, v in profiles.items() if not v.startswith('Custom profile:')}
...
@click.argument('profile_name')
@click.option("--embedding-type", type=click.Choice(['openai', 'sentence_transformers', 'cohere']),
              default='openai', help="Embedding type")
@click.option("--embedding-model", default="text-embedding-3-small", help="Embedding model name")
@click.option("--vectorstore-type", type=click.Choice(['chroma', 'faiss', 'pinecone']),
...
@click.option("--retriever-type", type=click.Choice(['similarity', 'mmr', 'ensemble']),
              default='similarity', help="Retriever type")
...
def add_profile(profile_name: str, embedding_type: str, embedding_model: str, 
                vectorstore_type: str, vectorstore_dir: Optional[str], 
                collection_name: Optional[str], retriever_type: str, 
                batch_size: int, activate: bool):
    ...
    try:
        ...
        new_profile = Config(
            embedding=EmbeddingConfig(
                type=EmbeddingType(embedding_type),
            ...
            try:
                ...
                if profile:
                    if profile in config_file.profiles:
                        config_obj = config_file.profiles[profile] 
                        logger.info(f"   Profile '{profile}': {config_obj.embedding.type} embeddings")
                    else:
