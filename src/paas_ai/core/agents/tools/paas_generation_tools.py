...
def generate_networking_yaml(
    ...
    if security_groups:
        ...
        for sg in security_groups:
            config["security_groups"][sg["name"]] = {
                "rules": sg["rules"]
            }
    ...
    for service in services:
        ...
        if service_type == "ecs":
            ...
            service_config["health_check"] = {
                "command": ["CMD", "curl", "-f", f"http://localhost:{service.get('port', 80)}{health_check_path}"],
                "interval": 30,
                "timeout": 5,
                "retries": 3
            }
            ...
            if "web" in service_name.lower():
                service_config["user_data"] = """#!/bin/bash
yum update -y
yum install -y nginx
...
def generate_load_balancer_yaml(
    load_balancer_spec: Dict[str, Any],
...
def paas_manifest_generator_tool(design_specification: str, **kwargs) -> str:
    """
    Generate complete Cool Demo PaaS YAML manifests from a design specification.
    Use this tool when you receive a complete design specification from the Designer Agent.
    
    Args:
        design_specification: JSON string of the complete design specification
    ...
    if not design_specification or not isinstance(design_specification, str):
        return json.dumps({
            "error": "Invalid design_specification provided. Must be a non-empty JSON string.",
            "files": [],
            "summary": {},
            "deployment_notes": []
        })
    ...
    try:
        ...
        # Also validate the generated manifests
        ...
        # logger.error(f"Error in paas_manifest_generator_tool: {e}") # This line was commented out in the original file
        return json.dumps({
            "error": f"Failed to generate manifests: {str(e)}",
            "files": [],
            "summary": {},
