...
"""
    ...
    def ask_stream(self, question: str):
        ...
        if self.verbose:
            ...
            self.logger.info(
                f"Processing question in {self.mode} mode (streaming): {question[:100]}..."
            )
        ...
        try:
            ...
            for token, metadata in self.coordinator.stream(
                initial_state, config=runtime_config, stream_mode="messages", subgraphs=True
            ):
    ...
    def ask_stream_direct(self, question: str):
        ...
        try:
            ...
            for token, metadata in primary_agent.react_agent.stream(
